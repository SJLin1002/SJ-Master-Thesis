...
def all(delaytTime,car_num,road):
    ...
    def CenterofMass(data):
        ...
        for car in data:
            ...
            Center[2] += car[2]
        total_cars = len(data)
        # Center[0] = round(Center[0] / total_cars, 4)
        # Center[1] = round(Center[1] / total_cars, 4)
        # Center[2] = round(Center[2] / total_cars, 4)
        Center[0] = Center[0] / total_cars
        Center[1] = Center[1] / total_cars
        Center[2] = Center[2] / total_cars
        return Center
    ...
    def sim(dataS):
        center_point = CenterofMass(dataS)
    ...
    def SimAct(location_UAV,dataA):
        ...
        center_point = CenterofMass(dataA)
        ...
        if int(tS/timeslot)>=int((t+delaytTime)/timeslot):
            for i in range(int(tS/timeslot)):  # D2
                ...
                if i == (int((t+delaytTime)/timeslot)-1) :# D1
                    New_center = CenterofMass(dataA)
            center_pointA = CenterofMass(dataA) 
        else:
            for i in range(int((t+delaytTime)/timeslot)):  # D2
                dataA = actual_change_car_data(dataA)           
                if i == (int(tS/timeslot)-1) :# D1
                    center_pointA = CenterofMass(dataA)
            New_center = CenterofMass(dataA) 
...
# workbook.save(f'data/CM_{h}{car_num}.xlsx')

# road = 0 平面道路(G)，road = 1 高速公路(H)
if road == 0 : 
    workbook.save(f'data/CM_{g}{car_num}.xlsx')
    print(g)
else : 
    workbook.save(f'data/CM_{h}{car_num}.xlsx')
    print(h)
