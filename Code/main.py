...
delaytTime = 3 #(s)
location_UAV=(0,0,V_UAV)  #無人機及車輛初始位置
...
def create_car(car_num): 
    ...
    return data
car_num=5
data=create_car(car_num)
print("初始車輛位置 : ",data) 

#更新data,1秒?!
def change_car_data(data):
    for car  in data:
        #車輛X座標改變
...
print("Updated Car Positions at Time 0:", kmeans(create_car))
for i in range(1, int(delaytTime/0.5)+1):  # 假设模拟5个时间步骤
    # 更新車輛位置
